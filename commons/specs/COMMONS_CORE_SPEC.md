...
**Companion:** COMMONS_CORE_MANIFEST (the "why/what"), COMMONS_MCP_ARCHITECTURE_SPEC.md (the knowledge service), PATTERN_SPEC.md (v8.2)

---

## §1 Purpose of This Specification

The COMMONS_CORE_MANIFEST defines what a Commons Core is — why it exists, what a commons means, how the architecture works conceptually. This specification defines **how you build, fork, boot, and operate** a Commons Core instance. It is the build document.

| Document | Question | Audience |
|---|---|---|
| **COMMONS_CORE_MANIFEST** | What IS a commons? What IS the OS? | Everyone — vision, principles, architecture |
...
When you fork the Commons Core template, you get a complete operating environment — not just a repository. The fork includes the workspace, the governance runtime, the knowledge base, and the agent configuration. Everything you need to start operating a commons from minute one.
...
GitHub offers two mechanisms for creating a new repository from an existing one. Both work for Commons Core, but they differ in important ways:
...
```bash
#!/bin/bash
# Commons Core — Repository Setup
# Run after forking: ./commons/setup.sh

REPO=$(gh repo view --json nameWithOwner -q '.nameWithOwner')
echo "Setting up Commons Core for $REPO"
...
git remote add upstream https://github.com/commons-engineering/commons-core.git 2>/dev/null || true
...
Extension packs are the primary mechanism for delivering domain-specific knowledge to a commons. Any Commons Incubator can build and publish packs. The Commons Core defines the interface; the packs provide the content.
...
Providers register themselves in `instance/registry/providers/` via YAML profiles. In the commons-core template, these profiles are curated via Pull Request. The Commons MCP indexes them for discoverability.
...
The commons-core template ships with the `commons-engineering` packs pre-loaded in `extensions/commons-engineering/`. This ensures the OS is immediately functional without MCP connectivity ("batteries included"). The Commons MCP keeps packs current and provides additional packs from other providers on demand.
...
You are the Purpose Agent of [commons]-os — a commons operating on the Commons Core.
...
- **Collections loaded:** Commons Blueprint, Commons Engineer, Commons Core, Commons Place + [domain] value stream collections
...
| **On fork update** | Upstream Commons Core template has new release, agent flags drift | Posts GitHub Issue |
...
```yaml
# Commons Core Configuration
commons_os_version: "0.2"
forked_from: "commons-engineering/commons-core"
...
The boot sequence follows the materialisation sequence from the COMMONS_CORE_MANIFEST §10.1. The AGENT.md guides the human through each phase.

| Phase | What happens | Agent's role | Human's role |
|---|---|---|---|
| **1. Fork & Clone** | Fork the commons-core template → `[commons]-os`, clone locally | — (repository is inert) | Forks the template, clones to local machine (or opens Codespace/IDE) |
...
Every Commons Core instance can run a continuous improvement loop. This is a concrete GitHub Actions workflow that runs on a schedule and proposes improvements.
...
- [ ] Create `commons-engineering/commons-core` as a GitHub template repository
